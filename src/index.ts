...
async function handleSearchFabCards(query: string) {
  try {
    logger.info({
...
async function handleGetFabCardPrints(cardId: string) {
  try {
    logger.info({
...
async function handleGetCardDetail(cardId: string, printId?: string) {
  try {
    logger.info({
      ...
      if (request.method === "POST") {
        try {
          ...
          if (jsonrpcRequest.method === "tools/list") {
            ...
            let result;
            if (name === "search_fab_cards") {
              result = await handleSearchFabCards(args.query);
            } else if (name === "get_fab_card_prints") {
              result = await handleGetFabCardPrints(args.cardId);
            } else if (name === "get_card_detail") {
              result = await handleGetCardDetail(args.cardId, args.printId);
            } else {
          ...
          return new Response(JSON.stringify({
            jsonrpc: "2.0",
            id: (JSON.parse(await request.text()))?.id || null,
